...
- **Which Backend Framework are we choosing?:** (Express, NestJS, Django, Flask, Ruby on Rails,
  ...
  _Considerations: If we choose NestJS, we get strict typing and architecture out of the box (good for "Standard User Mgmt"). If we choose Django, we get a built-in Admin panel (very fast setup). Please pick what you are most comfortable and confident in :)_
...
- **Which specific library will we use for WebSockets?** (Socket.io, native WebSockets, etc.)
  ...
  - _Considerations: Socket.io is generally recommended for the "Game" module because it simplifies the "reconnection logic" requirement._
